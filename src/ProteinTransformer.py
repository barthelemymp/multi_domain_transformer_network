...
class Transformer(nn.Module):
    ...
    def decode(self, memory, trg, memorymask):
        trg_seq_length= trg.shape[0]
    ...
    # def decode_Doubleattention(self, memory_list, trg):
    #     trg_seq_length= trg.shape[0]
...
class ContextNetwork(nn.Module):
    ...
    def decodeAll(self,batch, reconstruct, sizeTable, memorymask_list):
        ...
        for targetDecode in range(len(self.transformer_list)):
              if domainpresent[targetDecode]:
                ...
                ind = ind.to(self.transformer_list[targetDecode].device)
                ...
                print(batch[targetDecode][0].shape,  self.batchdim, ind.shape)
    ...
    def stackOnBatch(self, data):
        if self.batch_first:
            return torch.stack(data, 0)
        else:
            return torch.stack(data, 1)
        
    def stackOnLength(self, data):
        if self.batch_first:
            return torch.stack(data, 1)
        else:
            return torch.stack(data, 0)
